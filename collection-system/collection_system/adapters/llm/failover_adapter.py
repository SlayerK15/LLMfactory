...
class FailoverLLMAdapter:
    ...
    async def validate_urls(
        ...
        try:
            return await self._primary.validate_urls(topic=topic, query=query, items=items)
        except LLMRateLimitError:
            log.warning("llm.failover", operation="validate_urls", fallback="enabled")
            return await self._fallback.validate_urls(topic=topic, query=query, items=items)
